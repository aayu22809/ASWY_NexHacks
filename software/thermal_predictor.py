...
# Debug helper: set TP_DEBUG_PATH=1 to print the exact script path + hash being executed.
if os.environ.get("TP_DEBUG_PATH") == "1":
    try:
        import pathlib, hashlib
        _p = pathlib.Path(__file__).resolve()
        _h = hashlib.md5(_p.read_bytes()).hexdigest()
        print(f"[thermal_predictor] RUNNING: {_p} md5={_h}")
    except Exception as _e:
        print(f"[thermal_predictor] DEBUG_PATH failed: {_e}")


class SimulatedTempSource:
    """Fallback temperature source for laptops (no I2C/sensor).

    Produces a plausible temperature curve that responds to agent actions.
    """

    def __init__(self, start_c: float = 36.5):
        self.obj_temp = float(start_c)
        self.amb_temp = 24.0
        self._last_t = time.time()

    def step(self, planned_speed: float, plasma_power: float, standoff_mm: float):
        # very simple dynamics: power heats, standoff cools, time constant smoothing
        now = time.time()
        dt = max(now - self._last_t, 1e-3)
        self._last_t = now

        # heating rate scales with power and inversely with standoff
        heat = (plasma_power / 100.0) * (10.0 / max(standoff_mm, 1.0))
        # movement slightly reduces dwell heating
        heat *= 1.0 / (1.0 + (planned_speed / 60.0))

        # baseline cooling toward ambient
        cool = 0.15 * (self.obj_temp - self.amb_temp)

        dTdt = 0.55 * heat - cool
        self.obj_temp += dTdt * dt

        # clamp to reasonable demo range
        self.obj_temp = float(max(20.0, min(60.0, self.obj_temp)))

    def read_object_c(self) -> float:
        return float(self.obj_temp)

    def read_ambient_c(self) -> float:
        return float(self.amb_temp)
...
# Optional: fetch a public dataset (UCI) to sanity-check your training pipeline.
# IMPORTANT: do NOT fetch at import-time (breaks offline runs and can fail due to SSL).
try:
...
def fetch_uci_infrared_thermography(id: int = 925):
    """Fetch the UCI Infrared Thermography Temperature dataset (returns X, y, meta).

    Note: This requires internet access and a working SSL certificate chain.
    """
    if not _HAVE_UCI:
        raise RuntimeError("ucimlrepo not installed. Run: pip install ucimlrepo")
...
def main():
    ...
    p.add_argument("--simulate", action="store_true", help="Simulate temperatures (useful on Mac without MLX90614)")
    p.add_argument("--fetch-uci", action="store_true", help="Fetch UCI Infrared Thermography dataset (sanity check)")
    ...
    if args.fetch_uci:
        try:
            X, y, meta, vars_ = fetch_uci_infrared_thermography()
            print(meta)
            print(vars_)
            print("Fetched UCI dataset:", getattr(X, "shape", None), getattr(y, "shape", None))
        except Exception as e:
            print("Failed to fetch UCI dataset.")
            print("On macOS with python.org Python, you may need to run: Install Certificates.command")
            print("Error:", e)
        return
    ...
    if args.simulate:
        sim = SimulatedTempSource(start_c=36.5)
        print("Using simulated temperature source (--simulate)")
    ...
    if args.read_mlx:
        try:
            ...
            if sim is not None:
                print(f"Warning: failed to init MLX90614 ({e}). Falling back to simulation.")
                mlx = None
            else:
    ...
    if args.predict:
        ...
        ambient_temp = None
        if sim is not None:
            # take one dynamics step for a slightly non-trivial demo
            sim.step(planned_speed=60.0, plasma_power=80.0, standoff_mm=10.0)
            current_temp = sim.read_object_c()
            ambient_temp = sim.read_ambient_c()
    ...
    if args.loop:
        if mlx is None and sim is None:
            raise SystemExit("--loop requires --read-mlx (hardware) or --simulate (laptop demo)")
        ...
        try:
            while True:
                t = time.time()
                if sim is not None:
                    sim.step(planned_speed=planned_speed, plasma_power=plasma_power, standoff_mm=standoff_mm)
                    obj_temp = sim.read_object_c()
                    amb_temp = sim.read_ambient_c()
                else:
